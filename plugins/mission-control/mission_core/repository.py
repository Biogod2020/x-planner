...
from mission_core.state_machine import (
    ...
    require_accepted_evidence_for_terminal_task,
...
class MissionControlRepository:
    ...
    def update_task_status(self, task_id: int, status: TaskStatus | str) -> Task:
        ...
        accepted_evidence_count = self._task_accepted_evidence_count(task_id)
        require_accepted_evidence_for_terminal_task(status_value, accepted_evidence_count)
    ...
    def _task_accepted_evidence_count(self, task_id: int) -> int:
        row = self.connection.execute(
            "SELECT COUNT(*) AS count FROM evidence WHERE task_id = ? AND audit_status = 'accepted'",
            (task_id,),
        ).fetchone()
